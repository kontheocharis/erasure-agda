...
module Models.FO.ExtractionCorrectness where
...
embed-nat-inj ℕ.zero (suc m) p = exfalso-empty-prop (zeroΛ≠succΛ p)
embed-nat-inj (suc n) ℕ.zero p = exfalso-empty-prop (zeroΛ≠succΛ (sym p))
...
module _ where
  ...
  opaque
    ...
    fam-c .p,#q = refl
    fam-c .p∘,# {π = π} =
      Σ≡ refl (Σ≡ (funext (λ γₑ → exfalso-empty-prop (π γₑ)))
        (Σ≡ refl ( funext (λ γₛ → funext (λ γₑ → exfalso-empty-prop (π γₑ))))))
    fam-c .q[,#] = refl
    fam-c .↓ (xₛ , xₑ , x₀ , x₁) = xₛ , x₀
    fam-c .↓[] = refl
    fam-c .↑ (xₛ , x₀) = xₛ , (λ ()) , x₀ , λ γₛ ()
    fam-c .↑↓ {t = t} = Σ≡ refl (Σ≡ (funext (λ ()))
      (Σ≡ refl (funext (λ x → funext (λ y → exfalso-empty-prop y)))))
    fam-c .↓↑ = refl
